...
sf::Vector2f TransformMouse(sf::Vector2f pos) { return sf::Vector2f(pos * (1920.0f / sfmlState->videoMode.width)); }
...
int HandleInteraction(
	const sf::Vector2f& mPos, const sf::FloatRect& rect, bool& interactable, bool& containedLastFrame) {
	if (!interactable)
		return 0;
...
void DrawTextCentered(
	const sf::String& text, const sf::Vector2f& position, uint32_t size, sf::Color color, int fontIdx) {
	...
	sfmlState->texts[fontIdx].setPosition(
		2 * position.x - rect.left - rect.width * 0.5f, 2 * position.y - rect.top - rect.height * 0.5f);
	sfmlState->window.draw(sfmlState->texts[fontIdx]);
}
void DrawTextAligned(const sf::String& text, const sf::FloatRect& rect, uint32_t size, sf::Color color, int fontIdx,
	...
	int yPos = rect.top;
	if (hAlign == 2)
		xPos = rect.left + rect.width - bounds.width;
	else if (hAlign == 1)
		xPos = rect.left + (rect.width - bounds.width) * 0.5f;
	if (vAlign == 2)
		yPos = rect.top + rect.height - bounds.height;
	else if (vAlign == 1)
		yPos = rect.top + (rect.height - bounds.height) * 0.5f;
...
struct GenericRect : GuiObject {
	...
	GenericRect() = default;
	GenericRect(sf::FloatRect rect, sf::Color color = sf::Color::Transparent,
...
struct GenericTextRect : GenericRect {
	...
	GenericTextRect(sf::FloatRect rect, sf::String text, sf::Color textColor = sf::Color::White, uint32_t textSize = 24,
		int hAlign = 1, int vAlign = 1, int fontIdx = 0)
		: GenericRect(rect)
		, text(text)
		, textColor(textColor)
		, textSize(textSize)
	...
	GenericTextRect(GenericRect bg, sf::String text = "", sf::Color textColor = sf::Color::White,
		uint32_t textSize = 24, int hAlign = 1, int vAlign = 1, int fontIdx = 0)
		: GenericRect(bg)
		, text(text)
		, textColor(textColor)
		, textSize(textSize)
...
struct ImageRect : GuiObject {
	...
	void Render(const sf::FloatRect& window) { DrawTexture(generatedTex, image.w, image.h, AddWindow(window, rect)); }
};
...
struct InputRect : GuiInteractable {
	...
	void Render(const sf::FloatRect& window) {
		if (sfmlState->selectedText == &textInput)
			bg.outlineThickness = 1;
		else
			bg.outlineThickness = 0;
...
struct Checkbox : GuiInteractable {
	...
	Checkbox(sf::FloatRect rect, sf::Color bg, bool defaultVal = false)
		: GuiInteractable(rect), box(GenericRect(rect, bg), ""), enabled(defaultVal) {
		if (enabled)
			box.text = "X";
	}

	void Render(const sf::FloatRect& window) { box.Render(window); }
...
struct ScrollList : GuiInteractable {
	...
	ScrollList() = default;
	ScrollList(sf::FloatRect rect, GenericTextRect templateEntry,
		std::vector<sf::String> elements = std::vector<sf::String>(), float scrollbarWidth = 5)
	...
	void Render(const sf::FloatRect& window) {
		...
		dropdownView.setViewport(
			sf::FloatRect(tmprect.left / 1920, tmprect.top / 1080, tmprect.width / 1920, tmprect.height / 1080));
		...
		if (tmprect.height < elements.size() * templateEntry.rect.height) {
			...
			GenericRect(
				sf::FloatRect(tmprect.width - scrollbarWidth,
					(tmprect.height - scrollbarHeight) * scrollDistance / heightDiff, scrollbarWidth, scrollbarHeight),
		...
		//Outlines
		GenericRect(
			sf::FloatRect(0, 0, tmprect.width, tmprect.height), sf::Color::Transparent, sf::Color(60, 60, 60), 1)
			.Render(sf::FloatRect(0, 0, 0, 0));
		GenericRect(sf::FloatRect(0, selectedElement * templateEntry.rect.height - scrollDistance,
						templateEntry.rect.width, templateEntry.rect.height),
