...
static void stbhw__process_h_row(WangProcess& p, WangTileset& ts, int tx, int ty, int a0, int a1, int b0, int b1,
	int c0, int c1, int d0, int d1, int e0, int e1, int f0, int f1, int variants, int& i) {
...
static void stbhw__process_v_row(WangProcess& p, WangTileset& ts, int tx, int ty, int a0, int a1, int b0, int b1,
	int c0, int c1, int d0, int d1, int e0, int e1, int f0, int f1, int variants, int& i) {
	for (char v = 0; v < variants; ++v)
		for (char f = f0; f <= f1; ++f)
			for (char e = e0; e <= e1; ++e)
			...
			for (j = 0; j < ts.num_color[1]; ++j) {
				for (i = 0; i < ts.num_color[0]; ++i) {
					for (q = 0; q < ts.num_vary[1]; ++q) {
						stbhw__process_h_row(p, ts, 0, ty, 0, ts.num_color[1] - 1, 0, ts.num_color[2] - 1, 0,
							ts.num_color[3] - 1, i, i, j, j, k, k, ts.num_vary[0], hi);
						ty++;
					}
				}
			}
			...
			for (j = 0; j < ts.num_color[0]; ++j) {
				for (i = 0; i < ts.num_color[1]; ++i) {
					for (q = 0; q < ts.num_vary[0]; ++q) {
						stbhw__process_v_row(p, ts, 0, ty, 0, ts.num_color[0] - 1, 0, ts.num_color[3] - 1, 0,
							ts.num_color[2] - 1, i, i, j, j, k, k, ts.num_vary[1], vi);
						ty++;
					}
				}
			}
		...
		for (k = 0; k < ts.num_color[3]; ++k) {
			for (j = 0; j < ts.num_color[4]; ++j) {
				for (i = 0; i < ts.num_color[2]; ++i) {
					for (q = 0; q < ts.num_vary[1]; ++q) {
						stbhw__process_h_row(p, ts, 0, ty, 0, ts.num_color[2] - 1, k, k, 0, ts.num_color[1] - 1, j, j,
							0, ts.num_color[0] - 1, i, i, ts.num_vary[0], hi);
						ty++;
					}
				}
			}
		...
		for (k = 0; k < ts.num_color[3]; ++k) {
			for (j = 0; j < ts.num_color[4]; ++j) {
				for (i = 0; i < ts.num_color[5]; ++i) {
					for (q = 0; q < ts.num_vary[0]; ++q) {
						stbhw__process_v_row(p, ts, 0, ty, 0, ts.num_color[0] - 1, i, i, 0, ts.num_color[1] - 1, j, j,
							0, ts.num_color[5] - 1, k, k, ts.num_vary[1], vi);
						ty++;
					}
				}
			}
...
static Vec2i stbhw_get_index_stride(WangTile* list, int numlist, char a, char b, char c, char d, char e, char f) {
	...
	for (int i = 0; i < numlist; ++i) {
		WangTile& h = list[i];
		if ((a < 0 || a == h.colors[0]) && (b < 0 || b == h.colors[1]) && (c < 0 || c == h.colors[2]) &&
			(d < 0 || d == h.colors[3]) && (e < 0 || e == h.colors[4]) && (f < 0 || f == h.colors[5])) {
			//printf("%i ", i);
			if (first < 0)
				first = i;
			else if (second < 0)
...
_universal static uint32_t stbhw_get_index_num(char a, char b, char c, char d, char e, char f) {
	constexpr int base = 3;
	return base * base * base * base * base * a + base * base * base * base * b + base * base * base * c +
		   base * base * d + base * e + f;
}
...
#if 1
_compute static int stbhw__choose_tile(const WangTile* list, const uint16_t* indices, int max_colors, int numVary,
	WorldgenPRNG& prng, signed char& a, signed char& b, signed char& c, signed char& d, signed char& e,
	signed char& f) {
...
_compute static int stbhw__choose_tile(const WangTile* list, const uint16_t* indices, int max_colors, int numlist,
	WorldgenPRNG& prng, signed char& a, signed char& b, signed char& c, signed char& d, signed char& e,
	signed char& f) {
	int m = 1 << 30;
...
_compute static int stbhw__change_color(int old_color, int num_options, WorldgenPRNG& prng) {
	...
	if (scope.ts.is_corner) {
		...
		for (int j = 0; j < ymax - 3; ++j) {
			for (int i = 0; i < xmax - 3; ++i) {
				...
				if (stbhw__match(i, j, c_color) && stbhw__match(i, j + 1, c_color) && stbhw__match(i, j + 2, c_color) &&
					stbhw__match(i + 1, j, c_color) && stbhw__match(i + 1, j + 1, c_color) &&
					stbhw__match(i + 1, j + 2, c_color)) {
					int p = ((i + 1) - (j + 1) + 1) & 3;
					if (cc[p] > 1) {
				...
				if (stbhw__match(i, j, c_color) && stbhw__match(i + 1, j, c_color) && stbhw__match(i + 2, j, c_color) &&
					stbhw__match(i, j + 1, c_color) && stbhw__match(i + 1, j + 1, c_color) &&
					stbhw__match(i + 2, j + 1, c_color)) {
					int p = ((i + 2) - (j + 1) + 1) & 3;
					if (cc[p] > 1) {
		...
		for (int j = -1; yIdx < scope.bSec.wang_h; ++j) {
			...
			for (;; i += 4) {
				...
				if (xIdx >= scope.bSec.wang_w)
					break;
				if (xIdx + 2 >= 0 && yIdx >= 0) {
					int ti = stbhw__choose_tile(scope.ts.hTiles, scope.ts.hIndices, scope.ts.max_colors,
						scope.ts.num_vary[0] * scope.ts.num_vary[1], prng, c_color[j + 2][i + 2], c_color[j + 2][i + 3],
						c_color[j + 2][i + 4], c_color[j + 3][i + 2], c_color[j + 3][i + 3], c_color[j + 3][i + 4]);
					if (ti == -1)
						return 0;
					if (xIdx >= 0)
						output[yIdx * yWidth + xIdx] = ti;
					if (xIdx + 1 >= 0)
						output[yIdx * yWidth + xIdx + 1] = ti | 0x8000;
				}
				xIdx += 3;
				if (xIdx < scope.bSec.wang_w) {
					int ti = stbhw__choose_tile(scope.ts.vTiles, scope.ts.vIndices, scope.ts.max_colors,
						scope.ts.num_vary[0] * scope.ts.num_vary[1], prng, c_color[j + 2][i + 5], c_color[j + 3][i + 5],
						c_color[j + 4][i + 5], c_color[j + 2][i + 6], c_color[j + 3][i + 6], c_color[j + 4][i + 6]);
					if (ti == -1)
						return 0;
		...
		for (j = -1; yIdx < scope.bSec.wang_h; ++j) {
			...
			for (;; i += 4) {
				...
				if (xIdx + 2 >= 0 && yIdx >= 0) {
					int ti = stbhw__choose_tile(scope.ts.hTiles, scope.ts.hIndices, scope.ts.max_colors,
						scope.ts.num_vary[0] * scope.ts.num_vary[1], prng, h_color[j + 2][i + 2], h_color[j + 2][i + 3],
						v_color[j + 2][i + 2], v_color[j + 2][i + 4], h_color[j + 3][i + 2], h_color[j + 3][i + 3]);
					if (ti == -1)
						return 0;
					if (xIdx >= 0)
						output[yIdx * yWidth + xIdx] = ti;
				...
				if (xIdx < scope.bSec.wang_w) {
					int ti = stbhw__choose_tile(scope.ts.vTiles, scope.ts.vIndices, scope.ts.max_colors,
						scope.ts.num_vary[0] * scope.ts.num_vary[1], prng, h_color[j + 2][i + 5], v_color[j + 2][i + 5],
						v_color[j + 2][i + 6], v_color[j + 3][i + 5], v_color[j + 3][i + 6], h_color[j + 4][i + 5]);
					if (ti == -1)
						return 0;
					if (yIdx >= 0)
						output[yIdx * yWidth + xIdx] = ti | 0x4000;
