...
_compute static bool MaterialRefEquals(Material reference, Material test) {
	if (reference == MATERIAL_NONE)
		return true;
	if (test == MATERIAL_NONE)
		return false;
...
_compute static bool FungalShiftEquals(
	...
	if (reference.fromFlask && !test.fromFlask)
		return false;
	if (reference.toFlask && !test.toFlask)
		return false;
	if (!MaterialEquals((Material)reference.from, (Material)test.from, ptrs, vars))
		return false;
	if (!MaterialEquals((Material)reference.to, (Material)test.to, ptrs, vars))
		return false;
	return true;
}
...
_compute static bool CheckStartingFlask(NollaPRNG& random, StartingFlaskConfig c) {
	...
	if (res <= 65) {
		...
		Material magic[] = {Material::ACID, Material::MAGIC_LIQUID_POLYMORPH, Material::MAGIC_LIQUID_RANDOM_POLYMORPH,
			Material::MAGIC_LIQUID_BERSERK, Material::MAGIC_LIQUID_CHARM, Material::MAGIC_LIQUID_MOVEMENT_FASTER};
		material = magic[random.Random(0, 5)];
	} else {
		res = random.Random(0, 100000);
		if (res == 666)
			material = Material::URINE;
		else if (res == 79)
			material = Material::GOLD;
		else if (random.Random(0, 1) == 0)
			material = Material::SLIME;
		else
			material = Material::GUNPOWDER_UNSTABLE;
	}
...
_compute static bool CheckStartingWands(NollaPRNG& random, StartingWandConfig c) {
	if (c.projectile != SPELL_NONE) {
		...
		if (rnd < 50) {
			...
			selectedProj = SPELL_LIGHT_BULLET;
		if (selectedProj != c.projectile)
			return false;
	}
	...
	if (c.bomb != SPELL_NONE) {
		...
		if (rnd < 50) {
			...
			selectedBomb = SPELL_BOMB;
		if (selectedBomb != c.bomb)
			return false;
	}
	return true;
}
...
_compute static bool CheckFungalShifts(NollaPRNG& random, FungalShiftConfig c) {
	...
	for (int i = 0; i < maxFungalShifts; i++) {
		...
		generatedShifts[i].from = fungalMaterialsFrom[pick_random_from_table_weighted(
			fungalProbsFrom, fungalSumFrom, fungalMaterialsFromCount, random, rnd)];
		generatedShifts[i].to = fungalMaterialsTo[pick_random_from_table_weighted(
			fungalProbsTo, fungalSumTo, fungalMaterialsToCount, random, rnd)];
...
_compute static bool CheckBiomeModifiers(NollaPRNG& random, BiomeModifierConfig c) {
	...
	for (int i = 0; i < 9; i++) {
		...
		if (i == 1)
			chance = 0.15f;
		if (random_next(0, 1, random, rnd) > chance)
			continue;
		modifiers[i] = (BiomeModifier)(pick_random_from_table_weighted(
										   biomeModifierProbs, biomeModifierProbSum, biomeModifierCount, random, rnd) +
									   1);
	}
	for (int i = 0; i < 9; i++)
		if (c.modifiers[i] != BM_NONE && modifiers[i] != c.modifiers[i])
			return false;
	return true;
}
...
_compute static bool CheckPerks(NollaPRNG& random, PerkConfig c) {
	...
	for (int i = 0; i < maxPerkCount; i++)
		if (perkDeck[i] != 0)
			perkDeck[perkDeckIdx++] = perkDeck[i];
...
_compute bool PrecheckSeed(uint32_t seed, StaticPrecheckConfig c) {
	...
	if (c.cart.check)
		if (!CheckCart(sharedRandom, c.cart))
			return false;

	if (c.flask.check)
		if (!CheckStartingFlask(sharedRandom, c.flask))
			return false;

	if (c.wands.check)
		if (!CheckStartingWands(sharedRandom, c.wands))
			return false;

	if (c.alchemy.check)
		if (!CheckAlchemy(sharedRandom, c.alchemy))
			return false;

	if (c.rain.check)
		if (!CheckRain(sharedRandom, c.rain))
			return false;

	if (c.biomes.check)
		if (!CheckBiomeModifiers(sharedRandom, c.biomes))
			return false;

	if (c.fungal.check)
		if (!CheckFungalShifts(sharedRandom, c.fungal))
			return false;

	if (c.perks.check)
		if (!CheckPerks(sharedRandom, c.perks))
			return false;
